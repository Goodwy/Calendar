...
object Formatter {
    ...
    private const val DAY_OF_WEEK_LONG_PATTERN = "EEEE"
    ...
    fun getDateFromCode(context: Context, dayCode: String, shortMonth: Boolean = false, revers: Boolean = false): String {
        ...
        var date = if (revers) "$day $month" else "$month $day"
    ...
    fun getDayOfWeekTitle(dayCode: String): String {
        val dateTime = getDateTimeFromCode(dayCode)
        return dateTime.toString(DAY_OF_WEEK_LONG_PATTERN)
    }
    ...
    fun getMonthName(context: Context, id: Int) = context.resources.getStringArray(com.goodwy.commons.R.array.months)[id - 1]

    fun getShortMonthName(context: Context, id: Int) = context.resources.getStringArray(com.goodwy.commons.R.array.months_short)[id - 1]
