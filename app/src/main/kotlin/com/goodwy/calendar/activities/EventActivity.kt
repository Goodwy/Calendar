...
import com.goodwy.calendar.adapters.AutoCompleteTextViewAdapter
import com.goodwy.calendar.databinding.ActivityEventBinding
import com.goodwy.calendar.databinding.ItemAttendeeBinding
...
class EventActivity : SimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (baseConfig.backgroundColor != 0xFFFFFFFF.toInt()) setupMaterialScrollListener(binding.eventNestedScrollview, binding.eventAppbar)
    ...
    override fun onResume() {
        ...
        if (baseConfig.backgroundColor == white) {
            ...
            binding.eventToolbar.setBackgroundColor(colorToWhite)
        } else {
            window.decorView.setBackgroundColor(properBackgroundColor)
            binding.eventToolbar.setBackgroundColor(properBackgroundColor)
        }

        val holderBg = if (baseConfig.backgroundColor == white || baseConfig.backgroundColor == gray) white else getSurfaceColor()
        arrayOf(
            binding.eventTitleHolder,
            binding.eventTimeHolder,
            binding.eventRepetitionsHolder,
            binding.eventReminderHolder,
            binding.eventStatusesHolder,
            binding.eventTypesHolder,
            binding.eventAttendeesWrapperHolder,
        ).forEach {
        ...
        arrayOf(
            ...
            binding.eventStartDate,
            binding.eventEndTime,
            binding.eventEndDate,
        ).forEach {
    ...
    private fun maybeShowUnsavedChangesDialog(discard: () -> Unit) {
        ...
        if (now - mLastSavePromptTS > SAVE_DISCARD_PROMPT_INTERVAL && isEventChanged()) {
            ...
            ConfirmationAdvancedDialog(
                activity = this,
                message = "",
                messageId = com.goodwy.commons.R.string.save_before_closing,
                positive = com.goodwy.commons.R.string.save,
                negative = com.goodwy.commons.R.string.discard
            ) {
                if (it) {
                    saveCurrentEvent()
                } else {
    ...
    private fun gotEvent(
        ...
        eventTimeZoneHolder.setOnClickListener { setupTimeZone() }

        eventAllDay.setOnCheckedChangeListener { _, isChecked -> toggleAllDay(isChecked) }
        eventRepetitionHolder.setOnClickListener { showRepeatIntervalDialog() }
        ...
        eventReminder1Holder.setOnClickListener {
        ...
        eventReminder2Holder.setOnClickListener { showReminder2Dialog() }
        eventReminder3Holder.setOnClickListener { showReminder3Dialog() }
        ...
        eventAvailabilityHolder.setOnClickListener {
        ...
        eventStatusHolder.setOnClickListener {
    ...
    private fun checkRepeatTexts(limit: Int) {
        binding.eventRepetitionLimitDivider.root.beGoneIf(limit == 0)
        ...
        when {
            mRepeatInterval.isXWeeklyRepetition() -> {
                binding.eventRepetitionRule.text = if (mRepeatRule == EVERY_DAY_BIT) {
                    getString(com.goodwy.commons.R.string.every_day)
                } else {
    ...
    private fun updateReminder2Text() {
        binding.eventReminder2Holder.apply {
            beGoneIf(isGone() && mReminder1Minutes == REMINDER_OFF)
            binding.eventReminder2Divider.root.beGoneIf(this.isGone())
        }
    ...
    private fun showReminderTypePicker(currentValue: Int, callback: (Int) -> Unit) {
        val items = arrayListOf(
            RadioItem(REMINDER_NOTIFICATION, getString(com.goodwy.commons.R.string.notification)),
            RadioItem(REMINDER_EMAIL, getString(com.goodwy.commons.R.string.email))
        )
    ...
    private fun showStatusPicker(currentValue: Int, callback: (Int) -> Unit) {
        val items = arrayListOf(
            RadioItem(Events.STATUS_TENTATIVE, getString(R.string.status_tentative), icon = R.drawable.ic_question_circle_outline_vector),
            RadioItem(Events.STATUS_CONFIRMED, getString(R.string.status_confirmed), icon = R.drawable.ic_check_circle_outline_vector),
            RadioItem(Events.STATUS_CANCELED, getString(R.string.status_canceled), icon = R.drawable.ic_cancel_circle_outline_vector),
        )
        RadioGroupIconDialog(this, items, currentValue) {
            callback(it as Int)
        }
    }
    ...
    private fun showAvailabilityPicker(currentValue: Int, callback: (Int) -> Unit) {
        val items = arrayListOf(
            ...
            RadioItem(Attendees.AVAILABILITY_FREE, getString(R.string.status_free), icon = R.drawable.ic_event_available_vector)
        )
        RadioGroupIconDialog(this, items, currentValue) {
            callback(it as Int)
        }
    ...
    private fun updateReminderTypeImage(view: ImageView, reminder: Reminder) {
        ...
        val drawable = if (reminder.type == REMINDER_NOTIFICATION) {
            com.goodwy.commons.R.drawable.ic_bell_vector
        } else {
            com.goodwy.commons.R.drawable.ic_mail_vector
        }
    ...
    private fun showOrHideTimeZone() {
        ...
        binding.eventTimeZoneDivider.root.beVisibleIf(allowChangingTimeZones)
        binding.eventTimeZoneHolder.beVisibleIf(allowChangingTimeZones)
    }
    ...
    private fun addAttendee(attendee: Attendee? = null) {
        ...
        autoCompleteView.setColors(textColor, primaryColor, textCursorColor)
