...
import com.goodwy.calendar.databinding.ActivityTaskBinding
...
class TaskActivity : SimpleActivity() {
    ...
    override fun onResume() {
        ...
        if (baseConfig.backgroundColor == white) {
            ...
            binding.taskToolbar.setBackgroundColor(colorToWhite)
        } else {
        ...
        arrayOf(
            ...
            binding.taskRepetitionsHolder,
            binding.taskReminderHolder,
            binding.taskTypesHolder,
        ).forEach {
    ...
    private fun updateReminder2Text() {
        binding.taskReminder2Holder.apply {
            beGoneIf(isGone() && mReminder1Minutes == REMINDER_OFF)
            binding.taskReminder2Divider.root.beGoneIf(this.isGone())
        }
    ...
    private fun updateReminder3Text() {
        binding.taskReminder3Holder.apply {
            beGoneIf(isGone() && (mReminder2Minutes == REMINDER_OFF || mReminder1Minutes == REMINDER_OFF))
            binding.taskReminder3Divider.root.beGoneIf(this.isGone())
        }
...
//            binding.taskColorImage.beVisibleIf(calendar != null)
    ...
    private fun checkRepeatTexts(limit: Int) {
        binding.taskRepetitionLimitDivider.root.beGoneIf(limit == 0)
        binding.taskRepetitionLimitHolder.beGoneIf(limit == 0)
        checkRepetitionLimitText()

        val beVisible = mRepeatInterval.isXWeeklyRepetition() || mRepeatInterval.isXMonthlyRepetition() || mRepeatInterval.isXYearlyRepetition()
        binding.taskRepetitionRuleDivider.root.beVisibleIf(beVisible)
        binding.taskRepetitionRuleHolder.beVisibleIf(beVisible)
        checkRepetitionRuleText()
    }
